    ...
    <main className="pt-[92px]">
      <Section>
        <div className="grid-max max-w-4xl" data-animate="rise">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-mint-300">Talent bench</p>
        ...
        <div className="grid-max grid gap-6 md:grid-cols-3" data-batch="stagger">
          {openings.map((opening) => (
            <article
              key={opening.title}
              data-item
              className="glow-tile relative flex flex-col rounded-2xl border border-night-700/60 bg-night-900/60 p-6 backdrop-blur"
            >
              <div className="text-xs uppercase tracking-[0.3em] text-iris-300">{opening.type}</div>
              <h2 className="mt-3 text-xl font-semibold text-white">{opening.title}</h2>
              <p className="mt-2 flex-1 text-sm text-white/70">{opening.description}</p>
              <a
